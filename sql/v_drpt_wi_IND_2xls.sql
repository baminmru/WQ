drop view v_drpt_wi_ind
go
create view v_drpt_wi_ind
as
select
	  uds.FID_GRAPH,
	  track2obj.object_id,
...
drop view v_drpt_wi_ind_min 
go
create view v_drpt_wi_ind_min 
 as
--select min(T) minT,max(V) maxV,OBJECT_ID,DAYTYPE,DIRECTION  from v_drpt_wi group by OBJECT_ID,DAYTYPE,DIRECTION
select  avg(T) minT,avg(V) maxV,OBJECT_ID,DAYTYPE,DIRECTION  from v_drpt_wi_IND where  DAYINTERVAL=4 group by OBJECT_ID,DAYTYPE,DIRECTION
go

drop view v_drpt_wi_ind_2xls
go

create view v_drpt_wi_ind_2xls
as
...
from v_drpt_wi_ind v_drpt_wi join v_drpt_wi_ind_min v_drpt_wi_min on v_drpt_wi.object_ID=v_drpt_wi_min.object_id and v_drpt_wi.DAYTYPE=v_drpt_wi_min.DAYTYPE and v_drpt_wi.DIRECTION=v_drpt_wi_min.DIRECTION
where cnt >=8 and DAYINTERVAL >0 and FID_GRAPH !=0
go 


--select * from  v_drpt_wi_ind_2xls order by name,utoch,DAYTYPE,DAYINTERVAL,object_id,DIRECTION
