...
select
	  ...
	  where track2obj.object_id 	in (
		select  object_id from  kritical_objects
	)
		group by track2obj.object_id,uds.FID_GRAPH,
	  uds.name,uds.utoch, i.DIRECTION
	  ,uds.MLENGTH,i.DAYINTERVAL,i.DAYTYPE,datepart(hour,GPSTIME) , case
		  when datepart(minute,GPSTIME) <15 then '0-14'
		  when datepart(minute,GPSTIME) >= 15 and datepart(minute,GPSTIME) <30 then '15-29'
		  when datepart(minute,GPSTIME) >= 30 and datepart(minute,GPSTIME) <45 then '30-45'
		  when datepart(minute,GPSTIME) >= 45 then '45-59'
...
drop view v_krit_min 
go
create view v_krit_min 
 as
--select min(T) minT,max(V) maxV,OBJECT_ID,DAYTYPE,DIRECTION  from v_drpt_wi group by OBJECT_ID,DAYTYPE,DIRECTION
select  avg(T) minT,avg(V) maxV,OBJECT_ID,DAYTYPE,DIRECTION  from v_krit where  DAYINTERVAL=4 group by OBJECT_ID,DAYTYPE,DIRECTION
go

drop view v_krit_2xls
go

create view v_krit_2xls
as
...
from v_krit v_drpt_wi join  v_krit_min v_drpt_wi_min on v_drpt_wi.object_ID=v_drpt_wi_min.object_id and v_drpt_wi.DAYTYPE=v_drpt_wi_min.DAYTYPE and v_drpt_wi.DIRECTION=v_drpt_wi_min.DIRECTION
where DAYINTERVAL >0 and FID_GRAPH !=0
go 


select * from  v_krit_2xls order by name,utoch,DAYTYPE,DAYINTERVAL,object_id,H,Q,DIRECTION
